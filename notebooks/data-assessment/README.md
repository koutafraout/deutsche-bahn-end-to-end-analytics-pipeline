...
- **Monthly Parquet**: flat, already-typed tabular releases, as
  downloaded. Feeds the **monthly** reports, and is the MVP reporting
  source and reconciliation reference.
- **API raw collected data**: raw `timetables/v1/plan` and
  `timetables/v1/fchg` API responses (XML/JSON), collected hourly.
  Feeds the **daily** reports, and is the reference for the Bronze API
  schema and the candidate ~6-hourly ingestion source.
