...
Checks (see `validate.py` docstring for detail): valid Parquet, non-empty,
17-column schema match, non-null on the columns confirmed always-populated
in profiling, and `time` values falling inside their own source month.
