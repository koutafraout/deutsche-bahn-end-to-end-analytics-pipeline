...
marts schema in Redshift Serverless. Nothing here queries Bronze,
staging, or intermediate — the Redshift connection this script creates
is restricted (via schema filtering) to the `dbt_dev_marts` schema only.
...
- `setup_metabase.py` — builds the dashboard: Redshift connection, field
  remapping (so the Station filter searches by name but filters by
  `eva`), 3 tabs, 4 global filters (with Train Type → Line linked), and
  every card, laid out and wired with click-to-filter behavior.
